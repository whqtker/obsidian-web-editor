...
describe('replaceWikiLinks — 이미지 임베드 (![[...]])', () => {
  ...
  it('이중 ! 미생성 — 결과가 !! 로 시작하지 않아야 함', () => {
    ...
    expect(result).toBe('![photo.jpg](ghimg:attachments/photo.jpg)')
  })

  it('미해결 이미지 임베드 → 취소선', () => {
    const result = replaceWikiLinks('![[nonexistent.png]]', ALL_PATHS)
...
describe('replaceWikiLinks — 일반 위키링크 ([[...]])', () => {
  ...
  it('미해결 링크 → 취소선', () => {
    const result = replaceWikiLinks('[[없는노트]]', ALL_PATHS)
...
describe('replaceWikiLinks — 문장 내 혼합 사용', () => {
  ...
  it('위키링크와 이미지 임베드 혼합', () => {
    ...
    const result = replaceWikiLinks(input, ALL_PATHS)
    expect(result).toBe(
      '[My Note](wikilink:notes/My Note.md) 참고. ![photo.jpg](ghimg:attachments/photo.jpg)',
    )
  })
})
